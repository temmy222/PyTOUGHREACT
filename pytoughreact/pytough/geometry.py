...
def line_polygon_intersections(polygon, line, bound_line=(True, True),
    ...
    for i, p in enumerate(polygon):
        ...
        try:
            ...
            inline = True
            if bound_line[0]:
                inline = inline and (-tol <= xi[1])
            if bound_line[1]:
                inline = inline and (xi[1] <= 1.0 + tol)
    ...
    if indices:
        return [crossings[i] for i in sortindex], [ind[tuple(crossings[i])] for i in sortindex]
    else:
...
def polyline_line_distance(polyline, line):
    ...
    for i, pt in enumerate(polyline):
        ...
        if line_polygon_intersections(line, pline):
            return 0.0
        else:
            dists.append(min([point_line_distance(p, line) for p in pline]))
    return min(dists)
