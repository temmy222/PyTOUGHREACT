...
def sat(t, bounds=False):
    ...
    range."""
    a = [0., -7.691234564, -2.608023696e1, -1.681706546e2, 6.423285504e1, -1.189646225e2, 4.167117320, 2.097506760e1, 1.0e9, 6.0]
    if bounds:
        ok = (0.01 <= t <= Tc1_C)
    else:
        ok = True
...
def viss(t, d):
    ...
    V1 = 0.407 * t + 80.4
    if t <= 350.0:
        return 1.0e-7 * (V1 - d * (1858.0 - 5.9 * t) * 1.0e-3)
    else:
        return 1.0e-7 * (V1 + d * (0.353 + d * (676.5e-6 + d * 102.1e-9)))
