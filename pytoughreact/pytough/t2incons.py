...
class t2incon_parser(fixed_format_file):
    ...
    def __init__(self, filename, mode, read_function=fortran_read_function):
        super(t2incon_parser, self).__init__(filename, mode, t2incon_format_specification, read_function)
...
class t2incon(object):
    ...
    def __init__(self, filename='', read_function=fortran_read_function, num_variables=None):
        ...
        self.empty()
        if filename:
            self.read(filename, num_variables)

    def __getitem__(self, key):
        if isinstance(key, (int, slice)):
            return self._blocklist[key]
    ...
    def get_num_variables(self):
        if self.num_blocks > 0:
            return len(self._blocklist[0].variable)
        else:
            return 0
    ...
    def set_variable(self, val):
        ...
        for i, inc in enumerate(self._blocklist):
            inc.variable = val[i]
        ...
        if isinstance(por, np.ndarray):
            if len(por) == self.num_blocks:
                for i, p in enumerate(por):
                    self._blocklist[i].porosity = p
            else:
                raise Exception('Porosity array is the wrong length (' + str(len(por)) + ').')
        elif isinstance(por, float) or (por is None):
    ...
    def set_permeability(self, perm):
        if isinstance(perm, np.ndarray):
            ...
            if shape == (3,):
                ...
                for blk in self._blocklist:
                    blk.permeability = copy(perm)
            elif shape == (self.num_blocks, 3):
                for i, p in enumerate(perm):
                    self._blocklist[i].permeability = p
            else:
                raise Exception('Permeability array is the wrong shape (' + str(shape) + ').')
                ...
                if line.startswith('+++'):
                    ...
                    if valid_blockname(blkname):
                        ...
                        while more:
                            ...
                            while linevals and linevals[-1] is None:
                                linevals.pop()
                            vals += linevals
                        ...
                        raise Exception('Invalid block name (' + blkname + ') in incon file: ' + filename)
            else:
                finished = True
        self.timing = None
        if timing:
            ...
            if line.strip():
                line = padstring(line)
                timing_fmt = 'timing'
                if self.simulator == 'TOUGHREACT':
                    timing_fmt += '_toughreact'
    ...
    def write(self, filename, reset=True):
        ...
        for incon in self._blocklist:
            ...
            if self.simulator == 'TOUGHREACT' and incon.permeability is not None:
                outfile.write_values([blkname, incon.nseq, incon.nadd, incon.porosity] + list(incon.permeability), 'incon1_toughreact')
            else:
    ...
    def transfer_from(self, sourceinc, sourcegeo, geo, mapping={}, colmapping={}):
        ...
        if geo.atmosphere_type == 0:  # single atmosphere block
            ...
            if sourcegeo.atmosphere_type == 0:
                ...
                self[atmblk] = copy(default_atm_incons)
