...
class listingtable(object):
    ...
    def __getitem__(self, key):
        if isinstance(key, int):
            return dict(zip(['key'] + self.column_name, [self.row_name[key]] + list(self._data[key, :])))
        else:
            if key in self.column_name:
                ...
                return dict(zip(['key'] + self.column_name,
                                [self.row_name[rowindex]] + list(self._data[rowindex, :])))
                ...
                if revkey in self.row_name:
                    ...
                    return dict(zip(['key'] + self.column_name,
                                    [self.row_name[rowindex][::-1]] + list(-self._data[rowindex, :])))
            else:
                return None
    ...
    def __setitem__(self, key, value):
        if isinstance(key, int):
            ...
            self._data[self._row[key], :] = value
    ...
    def __add__(self, other):
        ...
        if self.column_name == other.column_name and self.row_name == other.row_name:
            ...
            raise Exception("Incompatible tables: can't be added together.")
    ...
    def __sub__(self, other):
        ...
        if self.column_name == other.column_name and self.row_name == other.row_name:
            ...
            raise Exception("Incompatible tables: can't be subtracted.")
...
class t2listing(object):
    ...
    def set_index(self, i):
        ...
        self._index = i
        if self._index < 0:
            self._index += self.num_fulltimes
        self.read_tables()
    ...
    def set_time(self, t):
        if t < self.fulltimes[0]:
            self.index = 0
        elif t > self.fulltimes[-1]:
            self.index = -1
        else:
    ...
    def get_num_fulltimes(self):
        return len(self.fulltimes)
    ...
    def set_step(self, step):
        if step < self.fullsteps[0]:
            self.index = 0
        elif step > self.fullsteps[-1]:
            self.index = -1
        else:
    ...
    def detect_simulator(self):
        ...
        if line != '' and self.simulator is None:
            ...
            line = self.readline()
            if line[1:].startswith('THE TIME IS'):
                line = self.readline()  # AUTOUGH2
            linechars = line[1:6]
            if linechars in simulator.keys():
                self.simulator = simulator[linechars]
                if self.simulator == 'TOUGH2' and MP:
                    self.simulator += '_MP'
        if self.simulator:
            ...
            for fname in internal_fns:
                ...
                if simname in ['TOUGH2_MP', 'TOUGHplus', 'TOUGHREACT', 'TOUGH3'] and not hasattr(self, fname_sim):
    ...
    def table_type_TOUGH2(self, headers):
        ...
        if headers[0:2] in [('ELEM.', 'INDEX'), ('ELEM.', 'IND.')]:
            if headers[2] == 'P':
                ...
                return 'primary'
        elif headers[0:3] == ('ELEM1', 'ELEM2', 'INDEX'):
            return 'connection'
    ...
    def table_type_TOUGHplus(self, headers):
        ...
        if headers[0:2] == ('ELEM', 'INDEX'):
            ...
            if headers in keytable:
                return keytable[headers]
        return None
    ...
    def setup_short_types(self):
        ...
        if self.simulator == 'AUTOUGH2':
            ...
            while not done:
                ...
                if (shortkw in self.short_types) or not shortkw:
                    done = True
                else:
            ...
            if len(shortpos) > 0:
                ...
                for itable, table in enumerate(self.short_types):
                    ...
                    if fulltable in self._table:
                        ...
                        def rowindex(line):
                            return fortran_int(line[indexpos: indexpos + 5]) - 1
                    self.skip_to_nonblank()
                    endtable = False
                    lineindex = 0
                    while not endtable:
                        line = self.readline()
                        if line[1:].startswith(table):
                            endtable = True
                        else:
                            index = rowindex(line)
    ...
    def setup_pos_AUTOUGH2(self):
        ...
        fullt, fulls, t, s = [], [], [], []
        keywords = ['EEEEE']
        if len(self.short_types) > 0:
            keywords.append(self.short_types[0])
        endfile = False
        while not endfile:
    ...
    def setup_pos_TOUGH2(self):
        ...
        while not endfile:
            ...
            while not (line.lstrip().lower().startswith('output data after') or line == ''):
                line = self.readline()
            if line != '':
                while not ('total time' in self.readline().lower()):
                    pass
                pos = self._file.tell()
    ...
    def setup_tables_AUTOUGH2(self):
        ...
        while tablename:
            self.read_header()
            if tablename in self.skip_tables:
                self.skip_table(tablename)
            else:
        ...
        while not found:
            ...
            if line == '':
                ...
                if (self.num_fulltimes > 1) and (self.index < self.num_fulltimes - 1):
                    if pos >= self._fullpos[self.index + 1]:
                        return None
                self.skip_to_nonblank()
    ...
    def skip_to_table_AUTOUGH2(self, tablename, last_tablename, nelt_tables):
        ...
        if self._short[self._index]:
            ...
            keyword, first_tablechar = tablechar * 5, 'E'
        if tablechar != first_tablechar:
            self.skipto(keyword)
        self.skipto('OUTPUT')
        self.skipto(keyword)
        self.skip_to_blank()
        ...
        if pt >= 2:
            ...
            if exponential:
                ...
                if pos > 0:
                    start = pos + 1
        if columns[0] == 'I':
            start -= 2  # e.g. ECO2M element table
        return start
    ...
    def setup_table_AUTOUGH2(self, tablename):
        ...
        if (len(cols) == nvalues):
            ...
            if keypos:
                ...
                raise Exception('Error parsing ' + tablename + ' table keys: table not created.')
        else:
        ...
        for i, pt in enumerate(pts[: -1]):
            ...
            if spacepos > 0:
                ...
                if exppos > 0:
                    ...
                    raise Exception("Unable to parse table line:\n" + line)
    ...
    def parse_table_header_TOUGH2(self):
        ...
        cols = []
        if self.simulator == 'TOUGH+':
            flow_headers = ['Flow', 'Veloc']
        else:
            flow_headers = ['RATE']
    ...
    def table_expected_floats(self, tablename, columns):
        ...
        if columns[0] == 'I':
            num_floats -= 1  # e.g. ECO2M element table
        return num_floats
    ...
    def setup_table_TOUGH2(self, tablename):
        ...
        if keypos:
            ...
            def count_read(count):
                return self.readline(), count + 1

            def is_header(line):
                return all([col in line for col in cols])

            def is_separator(line):
                return len(line) > lsep and line[1:lsep + 1] == line[1] * lsep
            while more:
                keyval = [fix_blockname(line[kp:kp + 5]) for kp in keypos]
                if len(keyval) > 1:
                    keyval = tuple(keyval)
                ...
                if len(line.strip()) > len(longest_line):
                    longest_line = line
                pos = self._file.tell()
                last_count = count
                ...
                if more and internal_header:
                    if internal_header_skiplines is None:
                        ...
                        for i in range(internal_header_skiplines):
                            line, count = count_read(count)
    ...
    def read_header_AUTOUGH2(self):
        ...
        try:
            self._step = fortran_int(line[istart:iend])
        except ValueError:
            self._step = -1  # to handle overflow
        istart = iend + 10
    ...
    def read_header_TOUGH2(self):
        ...
        strs = self.readline().split()
        if len(strs) < 4:
            self.skip_to_nonblank()  # to skip over extra lines in EOS7c listings
        else:
            self._file.seek(pos)
    ...
    def read_title_TOUGH2(self):
        ...
        if line == '':
            ...
            colonpos = line.find(':')
            if colonpos >= 0:
                self.title = line[colonpos + 1:].strip()
            else:
                self.title = ''
    ...
    def next_tablename(self, tablename):
        ...
        if tablename is None:
            return self._tablenames[0]
        i = self._tablenames.index(tablename)
        if i < len(self._tablenames) - 1:
            return self._tablenames[i + 1]
        else:
            return None
    ...
    def skip_table_TOUGH2(self, tablename):
        if tablename in self._table:
            ...
            if self.simulator == 'TOUGH+' and tablename == 'primary':
                chars = '_____'
            else:
                chars = '@@@@@'
            self.skipto(chars)
    ...
    def history(self, selection, short=True, start_datetime=None):
        ...
        if start_datetime is not None:
            ...
            def datetime_array(t):
                return np.array([start_datetime + timedelta(0, s) for s in t])
    ...
    def get_reductions(self):
        ...
        while not finished:
            ...
            if not finished:
                ...
                if eltindex > 0:
                    if lowerlastline.find('eos cannot find parameters') >= 0:
                        space = 9
                    else:
                        space = 8
    ...
    def get_vtk_data(self, geo, grid=None, flows=False, flux_matrix=None,
        ...
        for tablename in elt_tablenames:
            for name in self._table[tablename].column_name:
                arrays['Block'][name] = vtkFloatArray()
        flownames = []
        ...
        if flows:
            if flux_matrix is None:
                flux_matrix = grid.flux_matrix(geo, blockmap)
    ...
    def write_vtk(self, geo, filename, grid=None, indices=None, flows=False,
        ...
        if flows and (self.connection is not None):
            if grid is None:
                ...
                if geo_matches or len(blockmap) > 0:
                    doflows = True
                else:
                    raise Exception("t2listing.write_vtk(): if flows == True, " + "block names in the listing file and geometry must match, or " + "a block mapping must be specified.")
        arrays = geo.get_vtk_data(blockmap)
        if grid is not None:
            grid_arrays = grid.get_vtk_data(geo, blockmap)
            for array_type, array_dict in arrays.items():
                array_dict.update(grid_arrays[array_type])
        if doflows and flux_matrix is None:
            flux_matrix = grid.flux_matrix(geo, blockmap)
...
class t2historyfile(object):
    ...
    def detect_simulator(self, header):
        ...
        if 'OFT' in header:
            self.simulator = 'TOUGH2_MP'
        elif header.startswith('Time ['):
            self.simulator = 'TOUGH+'
        else:
    ...
    def setup_headers_TOUGH2(self, filename, header):
        ...
        if items[-1] == '':
            del items[-1]  # often an extra comma on the end of the lines
        items = items[3:]
        int_index = 0
...
class toughreact_tecplot(object):
    ...
    def __init__(self, filename, blocks):
        ...
        if self.num_times > 0:
            ...
            raise Exception('No results found in TOUGHREACT Tecplot file ' + filename)

    def __repr__(self):
        return "TOUGHREACT results for " + str(self.element.num_rows) + " blocks"
    ...
    def find_next_time(self):
        ...
        if quotepos >= 0:
            ...
            if spacepos >= 0:
                return float(line[quotepos + 1:spacepos]), num_lines
            else:
                return None, num_lines
        else:
            return None, num_lines
    ...
    def setup_table(self, blocks):
        ...
        if isinstance(blocks, mg.mulgrid):
            blocks = blocks.block_name_list
        elif isinstance(blocks, t2g.t2grid):
            blocks = [blk.name for blk in blocks.blocklist]
        ...
        if line is not None:
            ...
            raise Exception("Could not find variable definitions " + "for TOUGHREACT Tecplot file " + self.filename)
    ...
    def history(self, selection):
        ...
        def ordered_selection(selection):
            ...
            for sel_index, (key, h) in enumerate(selection):  # convert keys to indices as necessary
                if isinstance(key, int):
                    ...
                    if key in self.element.row_name:
                        index = self.element._row[key]
                    else:
                        index = None
                if index is not None:
                    osel.append((index, h, sel_index))
            osel.sort()
            return osel
        ...
        for ipos, pos in enumerate(self._pos):
            ...
            for (lineindex, colname, sel_index) in osel:
                if lineindex is not None:
                    for k in range(lineindex - index):
                        line = self._file.readline()
                    index = lineindex
    ...
    def get_vtk_data(self, geo, grid=None, geo_matches=True, blockmap={}):
        ...
        nele = geo.num_underground_blocks
        arrays = {'Block': {}, 'Node': {}}
        for name in self.element.column_name:
            arrays['Block'][name] = vtkFloatArray()
        array_length = {'Block': nele, 'Node': 0}
        array_data = {'Block': {}, 'Node': {}}
        ...
        for array_type, array_dict in arrays.items():
            for name, array in array_dict.items():
                ...
                array.SetNumberOfValues(array_length[array_type])
                if geo_matches:
                    array_data[array_type][name] = self.element[name][natm:]  # faster
                else:  # more flexible
